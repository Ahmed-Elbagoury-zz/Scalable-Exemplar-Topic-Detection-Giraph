...
public class VarianceAggregatorUnoptimized extends BasicAggregator<AggregateMessageCustome> {
  ...
  public static final String VAR_AGG = "VarianceAggregator";
  ...
  */
  ...
  public void aggregate(AggregateMessageCustome value) {
	...
	if(tweetList.size() == 1) { // worker aggregate
		...
		if(newList1.size() < topicNumber*factor) {
			newList1.add(currTweet);
			newList = newList1;
		} else {
			newList1.add(currTweet);
			...
			int lastTopic = -1;
			int topicCount = 0;
			Iterator<Tweet> tweetItr = newList1.iterator();
			Tweet nextTweet = tweetItr.next();
			Loop: while (topicCount < topicNumber*factor) {
				while(true) {	
					if(!nextTweet.getConflictList().contains(lastTopic)) {	
							break;
					}
					if (!tweetItr.hasNext())
						break Loop;
			...
			if (topicCount < topicNumber*factor) {
				for(Tweet tweet : newList1) {
					if(newList.size() == topicNumber*factor)
						break;
	...
	if(tweetList.size() > 1 && (stage % workerNum == 0) && (stage != 0) ){
		System.out.println("In the sorting aggregatoooooooooooooooor ...............................");
		try{
