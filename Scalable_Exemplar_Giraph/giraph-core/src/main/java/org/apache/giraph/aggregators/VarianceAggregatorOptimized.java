...
public class VarianceAggregatorOptimized extends BasicAggregator<AggregateMessageCustome> {
  ...
  public static final String VAR_AGG = "VarianceAggregatorOptimized";
  ...
  public void aggregate(AggregateMessageCustome value) {
	...
	if(tweetList.size() == 1) { // worker aggregate
		...
		try{
			...
			String outFileName = "/user/exp/ahmed/worker_aggregator_"+(Inet4Address.getLocalHost().getHostAddress())+".txt";
			Path pt = new Path(outFileName);
			BufferedWriter br  = null;
			StringBuilder content = new StringBuilder("");
			if (fs.exists(pt)) {
				Path ptRead = new Path(outFileName);
				...
				br.write(content.toString().trim());
				br.newLine();
			} else {
				br = new BufferedWriter(new OutputStreamWriter(fs.create(pt,true)));
			}
			br.write(currTweet.getId()+"\t"+currTweet.getVariance());
			br.newLine();
			br.close();
